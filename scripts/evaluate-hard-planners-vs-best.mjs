...
const BEST_SCRIPTED = Object.freeze([
  { charId: 'spearman', profile: 'heavy_bully', label: 'spearman:heavy_bully' },
...
function evaluatePlannerChar(charId, repeats) {
  const plannerProfile = PLANNER_PROFILES[charId];
  const rows = [];

  for (const opponent of BEST_SCRIPTED) {
    const row = initRow(opponent.label);
...
const outPath = path.resolve('analysis', `hard-planners-vs-best-${stamp}.json`);
ensureDir(outPath);
